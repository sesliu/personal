...
public class AulaWS {
	...
	@RequestMapping(value = "/relatorioFinanceiro/{mes}/{ano}/{lista}", method = RequestMethod.GET, produces = "application/json")
	public RetornaPDF geraRelatorioFinanceiro(@PathVariable("mes") String mes, @PathVariable ("ano") String ano, 
			                        		  @PathVariable("lista") String lista ) {
		...
		List<Aula> lst = new ArrayList<Aula>();
		try {
			 relatorio = aulaDao.geraRelatorioFinanceiro(mes, ano, lista);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
